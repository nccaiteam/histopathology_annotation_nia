...
bool XmlRepository::save() const
{
  ...
  pugi::xml_node root = xml.append_child("DP_Annotations");
  if (root.empty()) {
    return false;
  }
  ...
  std::wstring wSource = converter.from_bytes(_source);

  return xml.save_file(wSource.c_str());
}
...
bool XmlRepository::loadFromRepo()
{
  ...
  pugi::xml_parse_result tree = xml_doc.load_file(wSource.c_str());
  pugi::xml_node root = xml_doc.child("DP_Annotations");
  if (root.empty()) {
    root = xml_doc.root();
  }
